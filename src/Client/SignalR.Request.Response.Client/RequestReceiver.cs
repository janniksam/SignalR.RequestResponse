...
namespace SignalR.Request.Response.Client
{
    public class RequestReceiver : IRequestReceiver
    {
        ...
        private async Task<TResponse> GetResponse<TResponse>(Guid requestId) where TResponse : BaseResponse
        {
            ...
            {
                m_signalRConnection.Logger.LogAndThrow(new ConnectionLostException(string.Format("The connection was lost during the request with ID={0}", requestId), null));
            }
            ...
            do
            {
                ...
                {
                    m_signalRConnection.Logger.LogAndThrow(new RequestTimedOutException(string.Format("Request with the Id={0} timed out.", requestId)));
                }
            ...
            if (response.Aborted)
            {
                m_signalRConnection.Logger.LogAndThrow(new RequestAbortedException(string.Format("The request {0} was aborted by the server, ID: {1}", typeof(TResponse).Name, requestId)));
            }

            m_signalRConnection.Logger.LogInfo(string.Format("Response received: {0}, RequestID: {1}", typeof(TResponse).Name, requestId));
            var serializedResponse = Serializer.DeserializeObject<TResponse>(response.SerializedResponse);
            return serializedResponse;
        }

        public ISignalRConnection Connection => m_signalRConnection;
    }
}
